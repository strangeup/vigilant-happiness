...
CurvilineEllipseTop parametric_curve_top(2,1);
CurvilineEllipseBottom parametric_curve_bottom(2,1);
...
Outer_curvilinear_boundary_pt.resize(7);

//First bit
double zeta_start = MathematicalConstants::Pi/4;
double zeta_end = 7*MathematicalConstants::Pi/8;
unsigned nsegment = (int)(5*MathematicalConstants::Pi/(8*sqrt(outer_element_area)));
...
Outer_boundary_ellipse_pt->position(Vector<double>(1,zeta_end),vertices[0]);
vertices[1][0] = -1.0;
TriangleMeshPolyLine *boundary1_pt =
 new TriangleMeshPolyLine(vertices, Outer_boundary1);
Outer_curvilinear_boundary_pt[1] = boundary1_pt;

// Next Coordinate
zeta_start = 9*MathematicalConstants::Pi/8;
zeta_end = 7*MathematicalConstants::Pi/4;

// Third bit
vertices = Vector<Vector<double> > (2, Vector<double>(2,0.0));
vertices[0][0] = -1.0;
Outer_boundary_ellipse_pt->position(Vector<double>(1,zeta_start),vertices[1]);
TriangleMeshPolyLine *boundary2_pt =
 new TriangleMeshPolyLine(vertices, Outer_boundary2);
Outer_curvilinear_boundary_pt[2] = boundary2_pt;

// Fourth bit
nsegment = (int)(5*MathematicalConstants::Pi/(8*sqrt(outer_element_area)));
...
TriangleMeshPolyLine *boundary4_pt =
 new TriangleMeshPolyLine(vertices, Outer_boundary4);
Outer_curvilinear_boundary_pt[4] = boundary4_pt;
...
TriangleMeshPolyLine *boundary5_pt =
 new TriangleMeshPolyLine(vertices, Outer_boundary5);
Outer_curvilinear_boundary_pt[5] = boundary5_pt;

// Seventh bit
zeta_start = MathematicalConstants::Pi/4;
vertices = Vector<Vector<double> > (2, Vector<double>(2,0.0));
Outer_boundary_ellipse_pt->position(Vector<double>(1,zeta_start),vertices[1]);
vertices[0][0] = 11./5.;
vertices[0][1] = +3./2.;
TriangleMeshPolyLine *boundary6_pt =
 new TriangleMeshPolyLine(vertices, Outer_boundary6);
Outer_curvilinear_boundary_pt[6] = boundary6_pt;
...
{   
unsigned nbound = Outer_boundary5 + 1;
...
//  for(unsigned ibound=5;ibound<nbound;ibound++)
//  {
   ...
   for(unsigned n=0;n<nnode;++n)
    {
     ...
     // else if (nod_pt->is_on_boundary()) {/*Do nothing */}
...
{
// Number of plot points
npts = (TestSoln::high_resolution ? 30 : 6);
...
int main(int argc, char **argv)
{
 ...
 if(CommandLineArgs::command_line_flag_has_been_set("--high_resolution"))
  {TestSoln::high_resolution = true;}
...
problems for the curved edge elements. The prescribed area will be ignored."<<std::endl;
 ...
 // Initial doc
 TestSoln::problem_pt->doc_solution();
